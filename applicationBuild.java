...
public class applicationBuild extends JFrame {
	...
	// We use the static keyword here because we want these buttons to remain
	// the same throughout the entire program.
	// That means if I click btnView in a different class, we want to be
	// referencing the same button throughout the whole program.
	public static JButton btnView, btnEnter, btnClear, btnDelete;
	public static SpinnerModel sm;

	// If there is only one entry then when the user clicks the view button, the
	// database window does not display because there are
	...
	public applicationBuild() {
		...
		// Default values of weightx and weighty are 0, we need them to be 100
		...
		// TODO Things to be entered so far: package ID, package name, Database
		// ID(++ each package), date added, weight
		...
		// TODO Explain more in depth and in detail so that piazza knows about
		...
		// Added as += 1 so that no matter how many components we add, this one
		// will still remain at the bottom.
		gbc.gridy += 1;
		gbc.gridx = 0;
		gbc.gridwidth = 2;
		gbc.fill = GridBagConstraints.HORIZONTAL;
		...
		setResizable(false);
		setVisible(true);
		// This part of the JFrame building part sets the location of the window
